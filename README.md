# Federico Martínez - Perfil Profesional

# **Federico Martinez**

📱 +54 9 264 580 2870 | 📧 [fedemarti0107@gmail.com](mailto:fedemarti0107@gmail.com)

🌐 [LinkedIn](https://www.linkedin.com/in/federicoluismartinez/) | [Portfolio](https://fede-martinez-portfolio.vercel.app/) | [GitHub](https://github.com/fedemartinez5425) | [YouTube](https://www.youtube.com/@FedeMartinezQuantFinance)

---

## 🧠 **Resumen Profesional**

**Analista de Datos con enfoque en BI y Automatización de Procesos.**
Experiencia desarrollando soluciones de datos aplicadas a la toma de decisiones.
Capacidad demostrada para **resolver problemas de negocio usando Python, SQL y Tableau/Power BI**.
Interés en roles técnicos y de negocio. Disponible **full-time**.

---

## 💼 **Experiencia Relevante**

### Data Analyst Freelancer

**Fundación Sembrando Amor (Colombia)** | Jun 2025 - Sep 2025

* Limpieza, transformación y carga de datos desde documentos escaneados a **SQLite**.
* Creación de app básica en **Streamlit (Python)** para carga de datos desplegada en **Google Cloud Platform**.

### Analista y Cofundador

**Optima - Consulting & Management LLC** | Abr 2023 - Ene 2025

* Diseño de dashboards financieros y de negocio, reduciendo el tiempo de análisis en un **50%**.
* Automatización de reportes con **Python**, **Google Sheets** y **GCP**, reduciendo tareas repetitivas en un **40%**.
* Liderazgo en estrategias de datos para decisiones financieras con **Tableau/Power BI**.

### Administrativo - Optimización de Procesos

**Agua Santa Clara** | Ene 2023 - Abr 2023

* Automatización de tareas de facturación y control contable en **Excel** y **Google Sheets**.
* Gestión documental y atención a clientes corporativos.

---

## 🚀 **Proyectos Destacados**

* **Tablero de Opciones Financieras con Python y Streamlit**
  Visualización en tiempo real de datos del mercado de opciones.
  [App](https://optimaoptionspro.streamlit.app/)

* **Optimizador de Portfolios con Python y Streamlit**
  Simula carteras óptimas usando **Monte Carlo** y **Markowitz**.
  [App](https://portfoliopt.streamlit.app/)

* **Modelo de ML para Riesgo Crediticio**
  Predicción de morosidad con **árboles de decisión** y **Random Forest**.
  [App](https://creditscoremodel.streamlit.app/) | [Código](https://github.com/fedeheisemberg/Proyecto---Modelado-de-Riesgo-Crediticio)

* **Predicción de Rotación de Empleados con Python y Tableau**
  Modelo de churn y dashboard en **Tableau** para RRHH.
  [Proyecto](https://public.tableau.com/app/profile/federico.martinez3300/viz/DashboardML-HR/Dashboard1)

* **ETL Automatizado con Google Sheets API**
  Pipeline automatizado de datos bursátiles desde **Pyhomebroker** a **Google Sheets**.
  [Código](https://github.com/fedeheisemberg/Proyecto---Options-Data-with-Google-Sheets)

---

## 🎓 **Educación**

**Licenciatura en Administración de Empresas**
*Universidad de Congreso* | Mar 2023 - Actualidad (**88% completado**)

---

## 🏅 **Certificaciones**

* **Google Advanced Data Analytics** – Coursera - 2024
* **Associate Data Analyst** – DataCamp - 2024
* **Data Analytics Essentials** – Cisco Academy - 2024

---

## ⚙️ **Habilidades Técnicas**

Python | SQL | Tableau | Power BI | Excel (avanzado) | Google Sheets API | GCP | MySQL | SQLite | Git | Trello | Notion
*(Aprendiendo Apache Airflow y AWS)*

---

## 🌍 **Idiomas**

* **Inglés avanzado (C2)** – [Certificado EFSET](https://cert.efset.org/es/tH7DCc)

---

## 🎯 **Intereses**

* Business Intelligence y automatización
* Finanzas cuantitativas
* Divulgación educativa ([YouTube](https://www.youtube.com/@FedeMartinezQuantFinance))

---

